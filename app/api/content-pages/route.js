// Public Content Page API - Fetches page content from Unified CMS
// This is the public endpoint that StaticPage component uses
...
// Page Registry - mirrors the admin unified-pages registry
const PAGE_REGISTRY = {
  // Static/Info Pages
  homepage: { type: 'landing', title: 'Homepage', path: '/', category: 'Core' },
  about: { type: 'content', title: 'About Us', path: '/about', category: 'Company' },
  privacy: { type: 'content', title: 'Privacy Policy', path: '/privacy', category: 'Legal' },
  terms: { type: 'content', title: 'Terms of Service', path: '/terms', category: 'Legal' },
  cookies: { type: 'content', title: 'Cookie Policy', path: '/cookies', category: 'Legal' },
  contact: { type: 'content', title: 'Contact Us', path: '/contact', category: 'Company' },
  careers: { type: 'content', title: 'Careers', path: '/careers', category: 'Company' },
  faq: { type: 'content', title: 'FAQ', path: '/faq', category: 'Support' },
  
  // Feature Pages
  roadmap: { type: 'roadmap', title: 'Product Roadmap', path: '/roadmap', category: 'Product' },
  docs: { type: 'docs', title: 'Documentation', path: '/docs', category: 'Support' },
  
  // Solution Pages
  'solutions-creators': { type: 'solution', title: 'For Content Creators', path: '/solutions/creators', category: 'Solutions' },
  'solutions-marketers': { type: 'solution', title: 'For Marketing Teams', path: '/solutions/marketers', category: 'Solutions' },
  'solutions-agencies': { type: 'solution', title: 'For Agencies', path: '/solutions/agencies', category: 'Solutions' },
  'solutions-educators': { type: 'solution', title: 'For Educators', path: '/solutions/educators', category: 'Solutions' },
  
  // Other Pages
  security: { type: 'content', title: 'Security', path: '/security', category: 'Company' },
  status: { type: 'status', title: 'System Status', path: '/status', category: 'Support' },
  community: { type: 'content', title: 'Community', path: '/community', category: 'Support' }
}

// Default content templates for different page types
...
// GET - Fetch page content from unified CMS
export async function GET(request) {
  try {
    ...
    const unifiedCollection = db.collection(UNIFIED_PAGES_COLLECTION)
    const customCollection = db.collection(CUSTOM_PAGES_COLLECTION)
    
    // First, check unified_pages collection (admin-customized pages)
    let page = await unifiedCollection.findOne({ pageId })
    
    // If not found, check custom_pages collection (user-created pages)
    if (!page) {
    ...
    if (!page) {
      ...
      if (DEFAULT_TEMPLATES[pageId]) {
        const template = DEFAULT_TEMPLATES[pageId]
        const registryInfo = PAGE_REGISTRY[pageId] || {}
        
        page = {
          pageId,
          title: template.title || registryInfo.title || pageId,
          metaTitle: template.metaTitle || `${registryInfo.title || pageId} | ProCreators`,
          metaDescription: template.metaDescription || `${registryInfo.title || pageId} - ProCreators AI Content Creation Platform`,
          contentBlocks: template.contentBlocks || [],
          isPublished: true,
          isDefault: true
        }
      } else if (PAGE_REGISTRY[pageId]) {
        // Page is in registry but no template - create basic page
        const registryInfo = PAGE_REGISTRY[pageId]
        page = {
          pageId,
          title: registryInfo.title,
          metaTitle: `${registryInfo.title} | ProCreators`,
          metaDescription: `${registryInfo.title} - ProCreators AI Content Creation Platform`,
          contentBlocks: [
            {
              id: 'hero-1',
              type: 'hero',
    ...
    // Format response - ensure consistent structure for StaticPage component
    return NextResponse.json({ 
      success: true, 
      page: {
        pageId: page.pageId,
        title: page.title || page.seo?.metaTitle?.split('|')[0]?.trim() || pageId,
        metaTitle: page.metaTitle || page.seo?.metaTitle || `${page.title || pageId} | ProCreators`,
        metaDescription: page.metaDescription || page.seo?.metaDescription || '',
    ...
    console.error('Error fetching page from unified CMS:', error)
