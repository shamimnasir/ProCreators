...
export default function BlogPage() {
  ...
  return (
    <PublicLayout>
      {/* Header Section */}
      <section className="border-b border-border bg-background/80 backdrop-blur-xl">
          ...
          <div className="flex flex-wrap gap-3">
            {categories.map((category, index) => (
              <Button
                ...
                className={index === 0 ? 'bg-[#7c3aed] hover:bg-[#6d28d9]' : 'border-white/20 text-muted-foreground hover:bg-white/10'}
              >
                {category}
              </Button>
            ))}
      ...
      <section className="py-12">
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-foreground mb-2">Featured Post</h2>
            <div className="h-1 w-20 bg-gradient-to-r from-[#7c3aed] to-[#a78bfa] rounded"></div>
          </div>

          {blogPosts.filter(post => post.featured).map((post) => (
            <Card key={post.id} className="overflow-hidden border-border bg-gradient-to-b from-white/5 to-transparent hover:border-[#7c3aed]/50 transition-all">
              <div className="grid md:grid-cols-2 gap-0">
                <div className="relative h-64 md:h-auto bg-gradient-to-br from-[#7c3aed]/20 to-[#a78bfa]/20"></div>
                <div className="p-8">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-[#7c3aed] text-foreground">
                      {post.category}
                    </span>
                    <span className="text-sm text-muted-foreground">{post.readTime}</span>
                  </div>
                  <h3 className="text-3xl font-bold text-foreground mb-4">{post.title}</h3>
                  ...
                  <Button className="bg-gradient-to-r from-[#7c3aed] to-[#a78bfa] hover:from-[#6d28d9] hover:to-[#7c3aed] text-foreground">
                    Read Article
      ...
      <section className="py-12">
        <div className="container mx-auto px-6 max-w-6xl">
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-foreground mb-2">Recent Articles</h2>
            <div className="h-1 w-20 bg-gradient-to-r from-[#7c3aed] to-[#a78bfa] rounded"></div>
          </div>

          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {blogPosts.filter(post => !post.featured).map((post) => (
              <Card key={post.id} className="overflow-hidden border-border bg-gradient-to-b from-white/5 to-transparent hover:border-[#7c3aed]/50 transition-all group cursor-pointer">
                <div className="relative h-48 bg-gradient-to-br from-[#7c3aed]/20 to-[#a78bfa]/20"></div>
                <div className="p-6">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-white/10 text-muted-foreground">
                      {post.category}
                    </span>
                    <span className="text-xs text-gray-500">{post.readTime}</span>
                  </div>
                  <h3 className="text-xl font-bold text-foreground mb-3 group-hover:text-[#a78bfa] transition-colors">
                    {post.title}
                  </h3>
          ...
          <div className="mt-12 flex justify-center gap-2">
            <Button variant="outline" className="border-white/20 text-muted-foreground hover:bg-white/10">Previous</Button>
            <Button className="bg-[#7c3aed] hover:bg-[#6d28d9]">1</Button>
            <Button variant="outline" className="border-white/20 text-muted-foreground hover:bg-white/10">2</Button>
            <Button variant="outline" className="border-white/20 text-muted-foreground hover:bg-white/10">3</Button>
            <Button variant="outline" className="border-white/20 text-muted-foreground hover:bg-white/10">Next</Button>
          </div>
        </div>
      </section>
          ...
          <Card className="p-12 text-center bg-gradient-to-b from-[#7c3aed]/20 to-transparent border-[#7c3aed]/50">
            ...
            <div className="flex gap-4 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-foreground placeholder:text-gray-500 focus:outline-none focus:border-[#7c3aed]"
              />
              <Button className="bg-gradient-to-r from-[#7c3aed] to-[#a78bfa] hover:from-[#6d28d9] hover:to-[#7c3aed] text-foreground">
                Subscribe
              </Button>
