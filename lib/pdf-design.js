...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  if (hasImage) {
    // Small dark strip just behind the title/subtitle area - not covering the whole image
    page.drawRectangle({
      x: 0,
      y: 0,
      width: width,
      height: 280, // Only cover bottom portion where text appears
  ...
  if (authorName) {
    const authorFontSize = hasImage ? 18 : 16
    const authorY = hasImage ? 80 : 50
