...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  // Dark overlay at bottom for text readability (if image exists)
  if (hasImage) {
    // Dark gradient overlay at bottom half for title visibility
    page.drawRectangle({
      x: 0,
      y: 0,
      width: width,
  ...
  const decorColor = hasImage ? rgb(1, 1, 1) : (isDark ? colors.accent : colors.primary)
  
  // Title section (positioned in the safe lower area with dark overlay)
  const titleY = hasImage ? 200 : (height / 2 + 80)
