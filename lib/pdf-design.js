...
export function drawCornerDecorations(page, width, height, margin, colors, style) {
  const cornerSize = 40
...
export function drawDecorativeFrame(page, width, height, margin, colors) {
  // Outer frame
  page.drawRectangle({
    x: margin - 5,
    y: margin - 5,
...
export function drawPatternElements(page, width, height, colors, patternType) {
...
export function drawCoverPage(page, options) {
  ...
  if (coverStyle?.hasFrame) {
    drawDecorativeFrame(page, width, height, margin, colors)
  }
  
  // Draw corner decorations
  if (coverStyle?.cornerStyle) {
    drawCornerDecorations(page, width, height, margin + 15, colors, coverStyle.cornerStyle)
  }
  
  // Draw pattern if style has it
  if (coverStyle?.hasPattern) {
    drawPatternElements(page, width, height, colors, coverStyle.patternType || 'geometric')
  }
