...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  if (subtitle) {
    ...
    if (regularFont.widthOfTextAtSize(subtitle, subFontSize) > maxSubWidth) {
      displaySubtitle = subtitle.substring(0, 60) + '...'
    }
    const subtitleWidth = regularFont.widthOfTextAtSize(displaySubtitle, subFontSize)
    page.drawText(displaySubtitle, {
  ...
  // Author name
  if (authorName) {
    const byWidth = regularFont.widthOfTextAtSize('by', 11)
    page.drawText('by', {
      x: centerX - byWidth / 2,
      y: 62,
      size: 11,
    ...
    const authorWidth = boldFont.widthOfTextAtSize(authorName, 16)
  ...
  const brandWidth = regularFont.widthOfTextAtSize(brandText, 8)
  ...
  // Top decorative band (smaller if we have an image)
  const topBandHeight = hasImage ? 80 : 120
  page.drawRectangle({
    x: 0,
  ...
  page.drawLine({
    start: { x: margin, y: height - topBandHeight },
    end: { x: width - margin, y: height - topBandHeight },
    thickness: 3,
    color: decorColor,
  })
  
  // Draw frame if style has it
  if (coverStyle?.hasFrame && !hasImage) {
    drawDecorativeFrame(page, width, height, margin, colors, isDark)
  }
  
  // Draw corner decorations (always draw, they look good with images too)
  if (coverStyle?.cornerStyle) {
    drawCornerDecorations(page, width, height, margin + 15, colors, coverStyle.cornerStyle, isDark)
  }
  
  // Title positioning - lower if we have an image
  const titleY = hasImage ? 180 : (height / 2 + 80)
  ...
  page.drawLine({
    start: { x: centerX - 80, y: titleY - 50 },
    end: { x: centerX + 80, y: titleY - 50 },
    thickness: 1,
    color: subtitleColor,
  })
  
  // Year (if provided) - position depends on image
  if (year && !hasImage) {
    const yearText = year.toString()
    const yearWidth = boldFont.widthOfTextAtSize(yearText, 36)
    page.drawText(yearText, {
  ...
  // Year in bottom section if we have an image
  if (year && hasImage) {
    const yearText = year.toString()
    const yearWidth = regularFont.widthOfTextAtSize(yearText, 14)
    page.drawText(yearText, {
      x: width - margin - yearWidth,
      y: 45,
      size: 14,
      font: regularFont,
      color: subtitleColor,
