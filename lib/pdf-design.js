...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  // Try to embed and draw the cover image in FILL mode
  let hasImage = false
  if (coverImageUrl) {
    try {
      const embeddedImage = await embedImageFromDataUrl(pdfDoc, coverImageUrl)
      if (embeddedImage) {
        // FILL MODE: Cover entire top 60% of page, cropping to fit
        ...
        const areaAspectRatio = width / imageAreaHeight
        ...
        hasImage = true
        console.log('Cover image embedded in FILL mode')
      }
    } catch (error) {
  ...
  // Semi-transparent overlay at bottom for text readability (if image exists)
  if (hasImage) {
  ...
  // Title section (positioned in the safe lower area)
  const titleY = hasImage ? 250 : (height / 2 + 80)
  
  // Decorative line above title
  page.drawLine({
    start: { x: centerX - 80, y: titleY + 35 },
    end: { x: centerX + 80, y: titleY + 35 },
    thickness: 1,
    color: subtitleColor,
  })
  
  // Small diamond decoration
  const diamondY = titleY + 35
  page.drawLine({ start: { x: centerX - 6, y: diamondY }, end: { x: centerX, y: diamondY + 6 }, thickness: 1, color: decorColor })
  page.drawLine({ start: { x: centerX, y: diamondY + 6 }, end: { x: centerX + 6, y: diamondY }, thickness: 1, color: decorColor })
  page.drawLine({ start: { x: centerX + 6, y: diamondY }, end: { x: centerX, y: diamondY - 6 }, thickness: 1, color: decorColor })
  page.drawLine({ start: { x: centerX, y: diamondY - 6 }, end: { x: centerX - 6, y: diamondY }, thickness: 1, color: decorColor })
  ...
  const titleFontSize = title.length > 30 ? 24 : 28
  ...
  const safeDrawText = (page, text, options) => {
    try {
      ...
      // or skip if font truly doesn't support it
      try {
        // Try drawing with a simpler approach - split into smaller chunks
        const chunks = text.match(/.{1,5}/g) || [text]
        let xOffset = 0
        for (const chunk of chunks) {
          try {
            safeDrawText(page, chunk, { ...options, x: options.x + xOffset })
            xOffset += options.font.widthOfTextAtSize(chunk, options.size)
          } catch (chunkError) {
            // Skip problematic chunk
            xOffset += chunk.length * options.size * 0.5
          }
        }
        return true
      } catch (fallbackError) {
  ...
  const titleWidth = safeGetTextWidth(title, boldFont, titleFontSize)
  ...
  if (subtitle) {
    ...
    if (subWidth > maxSubWidth) {
      displaySubtitle = subtitle.substring(0, 60) + '...'
    }
  ...
  // Line below subtitle
  page.drawLine({
    start: { x: centerX - 60, y: titleY - 45 },
    end: { x: centerX + 60, y: titleY - 45 },
    thickness: 1,
    color: subtitleColor,
  })
  
  // Bottom band with author info
  page.drawRectangle({
    x: 0,
    y: 0,
    width: width,
  ...
  // Author name
  if (authorName) {
    const byWidth = safeGetTextWidth('by', 11, regularFont)
    safeDrawText(page, 'by', {
      x: centerX - byWidth / 2,
      y: 62,
      size: 11,
      font: regularFont,
    ...
    const authorWidth = safeGetTextWidth(authorName, 16, boldFont)
  ...
  const brandWidth = safeGetTextWidth(brandText, 8, regularFont)
...
export function drawCoverPage(page, options) {
  ...
  const titleWidth = safeGetTextWidth(title, 32, boldFont)
  safeDrawText(page, title, {
    x: centerX - titleWidth / 2,
    y: titleY,
    size: 32,
  ...
  // Subtitle
  if (subtitle) {
    const subtitleWidth = safeGetTextWidth(subtitle, 16, regularFont)
  ...
  if (year) {
    const yearText = year.toString()
    const yearWidth = safeGetTextWidth(yearText, 48, boldFont)
    safeDrawText(page, yearText, {
  ...
  if (authorName) {
    const byText = 'by'
    const byWidth = safeGetTextWidth(byText, 12, regularFont)
    safeDrawText(page, byText, {
      x: centerX - byWidth / 2,
      y: 70,
      size: 12,
      font: regularFont,
    ...
    const authorWidth = safeGetTextWidth(authorName, 18, boldFont)
    safeDrawText(page, authorName, {
      x: centerX - authorWidth / 2,
      y: 45,
      size: 18,
  ...
  const brandText = 'Created with ProCreators'
  const brandWidth = safeGetTextWidth(brandText, 9, regularFont)
  safeDrawText(page, brandText, {
    x: centerX - brandWidth / 2,
    y: 15,
    size: 9,
...
export function drawSectionPage(page, options) {
  ...
  if (pageNumber) {
    ...
    const numWidth = safeGetTextWidth(numText, 28, boldFont)
  ...
  // Title
  const titleWidth = safeGetTextWidth(title, 28, boldFont)
  safeDrawText(page, title, {
    x: centerX - titleWidth / 2,
    y: centerY - 20,
    size: 28,
  ...
  // Subtitle
  if (subtitle) {
    const subWidth = safeGetTextWidth(subtitle, 14, regularFont)
    safeDrawText(page, subtitle, {
