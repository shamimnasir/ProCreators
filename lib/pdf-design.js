...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  const decorColor = colors.secondary
  
  // Title section - positioned in lower third for visual balance
  const titleY = hasImage ? Math.min(180, height * 0.22) : (height / 2 + 80)
  ...
  // Note: baseWidth and scaleFactor already defined above
  ...
  // Draw compact blur/overlay strip just behind title and subtitle (if image exists)
  if (hasImage) {
    // Calculate the overlay size based on text dimensions
    const subtitleOffset = subtitle ? Math.floor(28 * scaleFactor) : 0
    const textBlockHeight = finalTitleFontSize + subtitleOffset + Math.floor(30 * scaleFactor) // padding
    const overlayPadding = Math.floor(20 * scaleFactor)
    const overlayWidth = Math.max(titleWidth, width * 0.7) + (overlayPadding * 2)
    const overlayHeight = textBlockHeight + (overlayPadding * 2)
    const overlayX = centerX - (overlayWidth / 2)
    const overlayY = titleY - subtitleOffset - overlayPadding - 5
    
    // Draw compact overlay strip behind title/subtitle
    page.drawRectangle({
      x: overlayX,
      y: overlayY,
      width: overlayWidth,
      height: overlayHeight,
      color: rgb(0.02, 0.02, 0.08), // Very dark overlay
      opacity: 0.6,
    })
  }
