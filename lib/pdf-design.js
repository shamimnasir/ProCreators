...
// Draw cover page with AI-generated image
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  // Try to embed and draw the cover image
  let hasImage = false
  if (coverImageUrl) {
    try {
      const embeddedImage = await embedImageFromDataUrl(pdfDoc, coverImageUrl)
      if (embeddedImage) {
        // Draw image as background covering most of the page
        const imageHeight = height * 0.55  // Cover 55% of page height
        const imageY = height - imageHeight - 80  // Position below top
