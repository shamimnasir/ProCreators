...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  let titleWidth
  try {
    titleWidth = boldFont.widthOfTextAtSize(title, titleFontSize)
  } catch (fontError) {
    console.error('Error measuring title width:', fontError.message)
    // Fallback to estimated width
    titleWidth = title.length * (titleFontSize * 0.5)
  }
  ...
  try {
    ...
    console.error('Error drawing title:', drawError.message)
    // Skip title drawing if font doesn't support characters
  }
