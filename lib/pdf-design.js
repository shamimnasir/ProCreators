...
export async function drawCoverPageWithImage(page, pdfDoc, options) {
  ...
  // Author name
  if (authorName) {
    const byWidth = safeGetTextWidth('by', regularFont, 11)
    ...
    const authorWidth = safeGetTextWidth(authorName, boldFont, 16)
  ...
  const brandWidth = safeGetTextWidth(brandText, regularFont, 8)
...
export function drawCoverPage(page, options) {
  ...
  if (year) {
    const yearText = year.toString()
    const yearWidth = safeGetTextWidth(yearText, boldFont, 48)
    safeDrawText(page, yearText, {
  ...
  if (authorName) {
    const byText = 'by'
    const byWidth = safeGetTextWidth(byText, regularFont, 12)
