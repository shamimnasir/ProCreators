...
export async function checkCredits(userId, toolId, params = {}) {
  ...
  if (isAdmin) {
    return {
      hasEnough: true,
      cost: 0, // No cost for admins
      currentBalance: 999999, // Display as unlimited
      shortfall: 0,
      plan,
      isAdmin: true
    }
...
export async function deductCredits(userId, toolId, params = {}) {
  ...
  if (user?.role === 'admin') {
    ...
    // Create a transaction record for tracking purposes (but no actual deduction)
    await db.collection('credit_transactions').insertOne({
      _id: transactionId,
      userId,
      toolId,
      amount: 0, // No deduction for admins
      status: 'completed',
      params,
      isAdmin: true,
      note: 'Admin - unlimited credits',
      createdAt: new Date(),
      completedAt: new Date()
    })
    
    // Update generation count for stats
