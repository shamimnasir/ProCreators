...
const RATE_LIMITS = {
  free: {
    ...
    concurrentGenerations: 2,   // Increased from 1 to give users more flexibility
...
export async function checkRateLimit(userId, plan = 'free') {
  ...
  if (rateLimitData && rateLimitData.concurrentGenerations > 0) {
    ...
    if (lastStart && lastStart < tenMinutesAgo && (!lastEnd || lastEnd < lastStart)) {
      console.log(`[RateLimit] Auto-resetting stuck concurrent generations for user ${userId}`)
