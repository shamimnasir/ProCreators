...
export async function generateConsistentVideoClips({
  ...
  // Calculate optimal number of clips (use 10s for cost efficiency)
  ...
  console.log(`[${jobId}] 🎬 Consistency mode: ${consistencyMode}`)
  ...
  for (let i = 0; i < numClips; i++) {
    ...
    try {
      ...
      if (consistencyMode === 'frame-chain' && i > 0 && lastFrameUrl) {
        ...
        // Note: I2V may have different duration limits, using 5s for safety
        console.log(`[${jobId}] Clip ${i + 1}: Using frame-chain (I2V, 5s)`)
        ...
        // Use text-to-video with optimal duration
        console.log(`[${jobId}] Clip ${i + 1}: Using text-to-video (${clipDuration}s)`)
        clip = await generateKlingClip({
          prompt: enhancedPrompt,
          aspectRatio,
      ...
      console.log(`[${jobId}] ✅ Clip ${i + 1}/${numClips} generated (${clipDuration}s)`)
