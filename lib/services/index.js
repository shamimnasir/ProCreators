...
export async function generateConsistentVideoClips({
  ...
  for (let i = 0; i < numClips; i++) {
    ...
    try {
      ...
      if (isMinimaxModel) {
        // Use Minimax for video generation
      ...
      clips.push({
        ...
        scenePrompt,
        enhancedPrompt,
        frameChained: !isMinimaxModel && consistencyMode === 'frame-chain' && i > 0,
        clipDuration,
        model: videoModel
      })
      
      console.log(`[${jobId}] ✅ Clip ${i + 1}/${numClips} generated with ${videoModel} (${clip.duration || clipDuration}s)`)
      
      // Extract last frame for next clip (if frame-chain mode with Kling)
      if (!isMinimaxModel && consistencyMode === 'frame-chain' && i < numClips - 1) {
        try {
