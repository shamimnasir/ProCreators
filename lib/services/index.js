...
const validateFalKeyFormat = (key) => {
  ...
  // KEY_ID should be a UUID (8-4-4-4-12 format) or similar identifier
  ...
  if (!uuidPattern.test(keyId)) {
    return { 
      valid: false, 
      error: `KEY_ID "${keyId.substring(0, 20)}..." doesn't look like a valid UUID. Did someone modify the key?`,
      hint: 'FAL keys should start with a UUID like: 360556a7-58bf-49f4-8ac8-dcb26a52d50d:...'
    }
  }
  
  // KEY_SECRET should be a 32-char hex string
  if (!/^[0-9a-f]{32}$/i.test(keySecret)) {
    return { 
      valid: false, 
      error: 'KEY_SECRET should be a 32-character hex string'
    }
  }
...
export const configureFal = () => {
  ...
  if (!validation.valid) {
    ...
    throw new Error(`FAL_KEY format invalid: ${validation.error}`)
  }
  ...
  if (lastConfiguredFalKey !== currentKey) {
    console.log('[FAL] Credentials configured/refreshed')
...
export async function generateConsistentVideoClips({
  ...
  console.log(`[${jobId}] 📝 Scene prompts mode: ${useScenePrompts ? 'AI-generated' : 'Raw prompt for all clips'}`)
  if (seedImageUrl) {
    console.log(`[${jobId}] 🖼️ Using seed image (${seedImageType}): ${seedImageUrl.substring(0, 60)}...`)
  }
  if (Object.keys(sceneReferenceUrls).length > 0) {
    console.log(`[${jobId}] 🖼️ Multi-scene references for scenes: ${Object.keys(sceneReferenceUrls).join(', ')}`)
  }
  ...
  if (useScenePrompts) {
    ...
    console.log(`[${jobId}] 📝 Using raw prompt for all clips: "${script.substring(0, 100)}..."`)
  ...
  for (let i = 0; i < numClips; i++) {
    ...
    try {
      ...
      const useSceneRef = sceneRefUrl && !isMinimaxModel
      const useSeedImage = !useSceneRef && seedImageUrl && i === 0 && !isMinimaxModel
      ...
      if (isMinimaxModel) {
        ...
        console.log(`[${jobId}] Clip ${i + 1}: Using Kling image-to-video with ${refType}`)
        ...
        if (consistencyMode === 'frame-chain') {
          try {
            ...
            console.log(`[${jobId}] Could not extract frame from reference image clip`)
          }
        }
      ...
      clips.push({
        ...
        scenePrompt,
        enhancedPrompt,
        frameChained: !isMinimaxModel && (imageToUse || (consistencyMode === 'frame-chain' && i > 0)),
        clipDuration,
        model: videoModel,
      ...
      console.log(`[${jobId}] ✅ Clip ${i + 1}/${numClips} generated with ${videoModel} (${clip.duration || clipDuration}s)${refInfo}`)
      
      // Extract last frame for next clip (if frame-chain mode with Kling)
      if (!isMinimaxModel && consistencyMode === 'frame-chain' && i < numClips - 1 && !imageToUse) {
        try {
          lastFrameUrl = await extractLastFrame(clip.url, tempDir, jobId)
        } catch (frameError) {
