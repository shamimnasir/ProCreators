...
export const getGemini = () => {
  if (!geminiInstance) {
    const key = process.env.EMERGENT_LLM_KEY || process.env.GOOGLE_API_KEY
    if (!key) throw new Error('EMERGENT_LLM_KEY or GOOGLE_API_KEY not configured')
...
export const isGeminiConfigured = () => !!(process.env.EMERGENT_LLM_KEY || process.env.GOOGLE_API_KEY)
