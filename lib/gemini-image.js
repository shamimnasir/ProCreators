...
export async function generateImage(prompt, model = "gemini-2.5-flash-image", quality = "standard", size = "1024x1024") {
  return new Promise((resolve) => {
    try {
      // Use Gemini Nano Banana script if Google API key is available, otherwise use DALL-E 3
      ...
      const inputData = JSON.stringify({
        prompt,
        model: useGemini ? 'gemini-2.5-flash-image' : model,
        quality,
        size
      })
