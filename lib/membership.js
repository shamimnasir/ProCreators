...
// SIMPLIFIED CREDIT SYSTEM v2: 1 credit ≈ $0.02
// Clean, round numbers for easy understanding
export const MEMBERSHIP_PLANS = {
  ...
  creator: {
    ...
    monthlyCredits: 1000, // ~66 AI images or 7 AI videos
    ...
    features: [
      '1,000 credits/month',
      '~66 AI images OR 7 AI videos (30s)',
      'All content tools',
      'No watermarks',
      'Bangla Voice Studio',
    ...
    creditDiscount: 0.05 // 5% off purchased credits
  },
  pro: {
    id: 'pro',
    name: 'Pro',
    ...
    monthlyCredits: 2500, // ~166 AI images or 19 AI videos
    ...
    features: [
      '2,500 credits/month',
      '~166 AI images OR 19 AI videos (30s)',
  ...
  business: {
    ...
    monthlyCredits: 5000, // ~333 AI images or 38 AI videos
    ...
    features: [
      '5,000 credits/month',
      '~333 AI images OR 38 AI videos (30s)',
      'Everything in Pro',
    ...
    creditDiscount: 0.15 // 15% off purchased credits
  }
}
...
export async function getUserMembership(userId) {
  ...
  const user = await db.collection('users').findOne(
    { _id: userId },
    { projection: { plan: 1, membershipCredits: 1, purchasedCredits: 1, subscriptionStatus: 1, subscriptionRenewsAt: 1, stripeSubscriptionId: 1, billingCycle: 1 } }
  )
...
export async function deductMembershipCredits(userId, amount, toolId, params = {}) {
  ...
  const user = await db.collection('users').findOne(
    { _id: userId },
    { projection: { membershipCredits: 1, purchasedCredits: 1 } }
  )
...
export async function refillMembershipCredits(userId) {
  ...
  const user = await db.collection('users').findOne(
    { _id: userId },
    { projection: { plan: 1, membershipCredits: 1, billingCycle: 1 } }
  )
